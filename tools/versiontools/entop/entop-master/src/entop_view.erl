...
loop(Parent, State) ->
	...
	{view, Direction} ->
	    case Direction of
			next -> State2 = update_sort_screen(State#state{line = State#state.line + 3}, State#state.sort);
			prev -> State2 = update_sort_screen(State#state{line = State#state.line - 3}, State#state.sort);
			top -> State2 = update_sort_screen(State#state{line = 0}, State#state.sort);
			bottom -> State2 = update_sort_screen(State#state{line = 999999999}, State#state.sort);
			pageDown -> State2 = update_sort_screen(State#state{line = State#state.line + 40}, State#state.sort);
			pageUp -> State2 = update_sort_screen(State#state{line = State#state.line - 40}, State#state.sort)
	    end,
	    loop(Parent, State2);
	reverse_sort ->
...
update_screen(Time, HeaderData, RowDataList, State) ->
    ...
    {State3,SortedRowList} = sort(RowList, State2),
    {Y, _} = cecho:getmaxyx(),
    StartY = (Y-(Y-7)),
    lists:foreach(fun(N) -> cecho:move(N, 0), cecho:hline($ , ?MAX_HLINE) end, lists:seq(StartY, Y)),
    update_rows(SortedRowList, State3#state.columns, State3#state.line, StartY, Y),
    cecho:refresh(),
    State3.
...
draw_title_bar(State) ->
    ...
    cecho:hline($ , ?MAX_HLINE),
    draw_title_bar(State#state.columns, 7),
...
print_showinfo(State, RoundTripTime) ->
    ...
    Showing = io_lib:format("Interval ~pms, Sorting on ~p (~s), Retrieved in ~pms Line:~p", 
			    [State#state.interval, ColName, SortName, RoundTripTime div 1000, State#state.line]),
...
sort(ProcList, State) ->
    ...
    L = case State#state.reverse_sort of
			true ->
	...
	if
		State#state.line < 0 ->
			{State#state{line = 0},L};
		State#state.line >= Size ->
			{_L1,L2} = lists:split(Size - 1,L),
			{State#state{line = Size - 1},L2};
		true ->
			{_L1,L2} = lists:split(State#state.line,L),
			{State,L2}
	end.

update_rows(ProcValuesList, _, _, LineNumber, Max) when LineNumber == Max orelse ProcValuesList == [] -> ok;
update_rows([RowValues|Rest], Columns, Line, LineNumber, Max) ->
	cecho:mvaddstr(LineNumber, 0, integer_to_list(Line + 1) ),
    update_row(tuple_to_list(RowValues), Columns, LineNumber, 6 + 1),
    update_rows(Rest, Columns, Line + 1, LineNumber + 1, Max).
