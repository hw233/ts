...
class BACKUP():
    #定义全局变量
    def __init__(self):
        self.dblist=['3dgameaccount',]
        self.mysql="/usr/local/mysql/bin/mysqldump -u root -p'wdnF-eg}:S#3Q)wA'  -h 127.0.0.1 -P3307  --opt --single-transaction --flush-logs"
        self.flushmysql="/usr/local/mysql/bin/mysql -u root -p'wdnF-eg}:S#3Q)wA' -h 127.0.0.1 -P3307 -e 'flush logs;'"
        self.export = '/usr/local/mysql/bin/mysqlbinlog'
        self.dir='/data/dbbackup'
        self.rsyncip = '183.61.84.168'
        self.passwd = '/etc/rsyncd.secrets'
        self.user = 'game_rsync'
        self.module = '3d_db_backup'
        self.binlog = '/data/log/mysql_binlog/mysql1'
	...
	global exectime
        ...
        tarcmd='cd %s && tar -zcf %s-%s.tar.gz %s.sql' %(self.dir,dbname,exectime,dbname)
	...
	try:
		if dbname == dblist[1]:
		    ...
		    self.backupcmd(dbname)
		    fullname='%s-%s.tar.gz' %(dbname,exectime)
		    self.Rsync(fullname)        
