...
header(SystemInfo, State) ->
	...
	Nodes = proplists:get_value(nodes, SystemInfo),

    Row1 = io_lib:format("Time: local time ~s, up for ~s, ~pms latency, nodes:~w",
			 [LocalTime, Uptime, PingTime,Nodes]),
...
row(ProcessInfo, State) ->
	...
	CurrentFunction = proplists:get_value(current_function, ProcessInfo, 0),
    {ok, {Pid, RegName, Reductions, Queue, Memory, Heap, Stack, HeapTot, Status, CurrentFunction}, State}.
