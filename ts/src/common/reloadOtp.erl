...
reload(Module) ->
  logger:error("Reloading ~p ..." , [Module]) ,
  case code:load_file(Module) of
    ...
    {error , Reason} ->
      logger:error(" ~ts: ~p, ~p" , ["code:load_file 加载模块代码出错" , Module , Reason]) ,
      error
  end.
...
immediately(Module) ->
  ...
  case erlang:function_exported(Module , test , 0) of
    true ->
      logger:info("reload ~p ok." , [Module]) ,
      reload;
    false ->
      reload
  end.
  ...
  lists:any(
    fun(P) ->
      case erlang:check_process_code(P , Module) of
        true ->
          logger:error("~ts:~p" , ["热代码更新的时候发现旧代码引用" , erlang:process_info(P)]) ,
          true;
        false ->
          false
      end
