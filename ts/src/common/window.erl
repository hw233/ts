...
createWindow(_Log_File_Name) ->
	log4erl:info("Cur Is Release Version"),
	ok.
-else.
createWindow(Log_File_Name) ->
	log4erl:info("Cur Is Debug Version"),
...
deal_fuc(Pid, Param)->
	try
		case Param of
			...
			UnkownMsg ->
                logger:error("window deal_fuc, recv unkown msg: ~p", [UnkownMsg])
		end
	catch
		_:_Why -> 
			logger:error("exception _Why: ~p, erlang:get_stacktrace: ~p", [_Why, erlang:get_stacktrace()])
	end.
...
handle_cast({insert_record, Text0, MsgLevel}, #window{isNeedAlter = IsNeedAlter} = State) ->
	Text = try
			...
			_:Why ->
				logger:error("insert_record failed:~p ~p ~p",[Why,Text0,misc:getStackTrace()]),
				""
		end,
    case MsgLevel of
		fatal ->
